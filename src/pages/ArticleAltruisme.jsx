...
function ArticleAltruisme() {
  const [datas, setData] = useState([]);

  useEffect(() => {
    axios("https://6480432af061e6ec4d48ebcc.mockapi.io/article-home?page=7&limit=1").then((result) => setData(result.data));
  }, []);
          ...
          <Row>
            {datas.map((item) => (
              <Col lg={{ span: 7, offset: 0 }} key={item.id}>
                <div>
                  <p id="breadcrumbs">Artikel : Kenali Altruisme, Sikap Kepedulian Tinggi pada Orang Lain</p>
                  <h3 id="main-title">{item.title}</h3>
                  <p id="date">{item.date}</p>
                  <img src={item.img} id="img-thumbnail"></img>
                  <br></br>
...
export default ArticleAltruisme;
